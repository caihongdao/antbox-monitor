...
class SiteScanner {
    ...
    setupEventListeners() {
        ...
        if (maxConcurrentInput) {
            ...
            maxConcurrentInput.addEventListener('change', (e) => {
                ...
                if (value > 999) {
                    e.target.value = 999;
                    (window.app ? window.app.showToast.bind(window.app) : console.log)('最大并发数不能超过999', 'warning');
                } else if (value < 1) {
                    e.target.value = 20; // 默认值
                    (window.app ? window.app.showToast.bind(window.app) : console.log)('最小并发数为1', 'warning');
                }
            });
        }
    }
    ...
    async startScan() {
        ...
        if (!this.isValidIP(startIP) || !this.isValidIP(endIP)) {
            ...
            (window.app ? window.app.showToast.bind(window.app) : console.log)('请输入有效的IP地址', 'error');
            return;
        }
        ...
        (window.app ? window.app.showToast.bind(window.app) : console.log)('扫描已开始', 'info');
    ...
    scanComplete() {
        ...
        (window.app ? window.app.showToast.bind(window.app) : console.log)(`扫描完成，发现 ${this.scanStats.foundDevices} 个设备`, 'success');
    ...
    stopScan() {
        ...
        (window.app ? window.app.showToast.bind(window.app) : console.log)('扫描已停止', 'warning');
    ...
    showImportModal() {
        ...
        if (selectedIPs.length === 0) {
            (window.app ? window.app.showToast.bind(window.app) : console.log)('请先选择要导入的设备', 'warning');
            return;
        }
    ...
    async confirmImport() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                (window.app ? window.app.showToast.bind(window.app) : console.log)(`成功导入 ${selectedIPs.length} 个设备`, 'success');
            ...
            (window.app ? window.app.showToast.bind(window.app) : console.log)('导入失败，请检查网络连接', 'error');
        }
    ...
    async testDevice(ip, port) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                (window.app ? window.app.showToast.bind(window.app) : console.log)(`设备 ${ip} 连接正常`, 'success');
            } else {
                throw new Error(`HTTP ${response.status}`);
            }
        } catch (error) {
            this.addLog(`设备 ${ip} 连接测试失败: ${error.message}`, 'error');
            (window.app ? window.app.showToast.bind(window.app) : console.log)(`设备 ${ip} 连接失败`, 'error');
        }
    }
    ...
    openDeviceDetail(ip) {
        ...
        if (!device) {
            (window.app ? window.app.showToast.bind(window.app) : console.log)('未找到设备信息', 'warning');
            return;
        }
    ...
    showDeviceInfo(ip) {
        ...
        infoHtml += `
            <div class="modal-actions">
                ...
                <button class="btn btn-secondary" onclick="(window.app ? window.app.closeModal.bind(window.app) : console.log)()">关闭</button>
            </div>
        `;
        
        (window.app ? window.app.showModal.bind(window.app) : alert)('设备信息', infoHtml);
    }
    ...
    async addToDatabase(ip) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                (window.app ? window.app.showToast.bind(window.app) : console.log)(`设备 ${ip} 添加成功`, 'success');
            } else {
                throw new Error('添加失败');
            }
        } catch (error) {
            this.addLog(`添加设备 ${ip} 失败: ${error.message}`, 'error');
            (window.app ? window.app.showToast.bind(window.app) : console.log)(`添加设备失败`, 'error');
        }
    }
